...
const establish = async () => {
  if(Address.value == null) return ElMessage.warning("Please understand the wallet operation first");
  ...
  let counter=null;
	let len = 0;
  try{
    counter= await contracts.ldc.account.creatorMemecoinCounter.fetch(memeCounterPda);
    len=counter.count;
    console.log("len is1",len)
  }catch (error) {
    len = 0;
  ...
  console.log('第二个',memeCoinConfigPda);
  ...
  console.log('第3个',mintPda);
  const [metaDataPda]=PublicKey.findProgramAddressSync(
    [new Buffer( "metadata"),toPublicKey(mpl_token_address).toBuffer(),mintPda.toBuffer()],
    toPublicKey(mpl_token_address)
    )
    console.log('第4个',metaDataPda);
    const [desPda] = PublicKey.findProgramAddressSync(
      [new Buffer("MEME_COIN"),mintPda.toBuffer(),memeCoinConfigPda.toBuffer()],
      toPublicKey(programId)
    )
    console.log('第5个',desPda);
  const [configPda] = PublicKey.findProgramAddressSync(
    [new Buffer("CONFIG")],
    toPublicKey(programId)
  );
  console.log('第6个',configPda);
  console.log('7个',tokenProgram2022);
  console.log('8个',mpl_token_address);
  console.log('这个是图片',imageUrl.value);
  try {
    ...
    let hash = await contracts.ldc.methods
      .createMemecoinConfig(
       ...
       Number(amount.value),
        ...
        createMemecoinFeeReceiver: toPublicKey("9d8Uy7Zpcs7zhE1sd6zS3azQDbLT1XCphXSiCdueq48C"),
        mint:mintPda,
        destination:desPda,
